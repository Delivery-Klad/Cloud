...
";
        ...
        private Particle GenParticle()
        {
            ...
            result.temperature = (float)(rnd.NextDouble() * (max_temperature - min_temperature)*(1- rnd.NextDouble()) + min_temperature);

            result.size = (float)(rnd.NextDouble()*2.5+0.5);
            result.position = new Vector3((float)(Math.Cos(rnd.NextDouble() * 180.0) * rnd.NextDouble()), 0.0f, (float)(Math.Sin(rnd.NextDouble() * 180.0) * rnd.NextDouble()));
