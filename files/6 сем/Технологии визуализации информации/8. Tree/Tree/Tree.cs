...
";
        ...
        int shader, shadow_shader;
        int vao, vbo;
        public int Texture, ShadowTexture;
        ...
        VertexData[] GenVertices()
        {
            ...
            foreach (var branch in allBranches)
            {
                ...
                float parentThickness = branch.Thickness;
        ...
        public void RenderShadow(Matrix4 lightProjection, Matrix4 lightModelView, int shadowTexture, int shadowSize)
        {
            ...
            GL.BindVertexArray(vao);
            GL.UseProgram(shader);
            GL.UniformMatrix4(GL.GetUniformLocation(shader, "modelview"), false, ref lightModelView);
            GL.UniformMatrix4(GL.GetUniformLocation(shader, "projection"), false, ref lightProjection);
            GL.BindTexture(TextureTarget.Texture2D, 0);
            GL.DrawArrays(PrimitiveType.Triangles, 0, totalVertices);
            GL.BindVertexArray(0);
